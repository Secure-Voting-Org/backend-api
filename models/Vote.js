...
const castVote = async (voterId, candidateId, constituency) => {
    ...
    const query = 'INSERT INTO votes (voter_id, candidate_id, constituency, transaction_hash) VALUES (?, ?, ?, ?)';
    await mysqlPool.query(query, [voterId, candidateId, constituency, transactionHash]);
...
const getPublicLedger = async (limit = 20) => {
    const query = 'SELECT transaction_hash, constituency, timestamp FROM votes ORDER BY timestamp DESC LIMIT ?';
